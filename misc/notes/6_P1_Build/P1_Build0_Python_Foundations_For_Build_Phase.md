...
**`mcp` vs. `fastmcp` package choice — not decided.** Two distinct packages now exist: the official MCP Python SDK (importing `mcp.server.fastmcp`) and the standalone `fastmcp` project, where FastMCP 2.0 is described as the successor to FastMCP 1.0, which itself became the MCP Python SDK. Their APIs have diverged. This should be decided deliberately at P1-Build-1 after checking current documentation for both — not settled by whichever tutorial happens to be open.
