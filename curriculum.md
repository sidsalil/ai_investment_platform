...
> Last updated: 2026-07-21 (P1-Arch-3 complete — Orchestrator + subagent
> design. Plan object fully instantiated for P1's 7 pipeline stages; exact
> validator/memo subagent handoff payload schemas locked; full subagent
> failure-propagation table designed (blocking severity escalates to a human
...
> decision). Before that, 2026-07-20
> (P1-Arch-1 complete — System design, full architecture diagram for
> Project 1 locked, including the MCP-vs-native-tool scope decision and the
> tracing-vs-product-metrics subsystem distinction). Before that, 2026-07-19 (P1-LE2 complete — AI product metrics & KPIs,
...
> completed — the first two lessons of the Backtesting Rigor track. Also earlier the same
> day, P1-LA17, Prompt engineering fundamentals, completed the
...
### [x] P1-LA15: RAG fundamentals — **completed 2026-07-16**
- **Concepts:** Retrieval-Augmented Generation (RAG) two-phase pipeline (offline indexing: chunking, embedding, vector database; query-time: embed query, similarity search, top-k retrieval, inject into context, generate), embeddings as meaning-space vectors, cosine similarity as the comparison metric, semantic search vs. keyword search, when RAG is (and is not) relevant to a research-copilot document-lookup use case
- **Deliverable:** Notes (P1_LA15_RAG_Fundamentals.md), including a worked 3-D toy numerical example computing cosine similarity between a query and three document chunks
...
## Phase 1: Concept Lessons — Backtesting Rigor Track (~4-6 hours) — COMPLETE (3/3)
...
### [x] P1-LB2: Out-of-sample vs. in-sample discipline — completed 2026-07-18
...
### [x] P1-LB3: The multiple-testing problem (p-hacking) — completed 2026-07-18
...
- **Explain back:** "I tested momentum, volatility, and value — why can't I just report whichever had the best backtest?" — answered in notes: "best backtest" is the output of a selection process that mechanically inflates the reported number in proportion to how many things were tested (family-wise error rate, worked table 5%→92.3% for n=1→50); fix is disclosing/correcting for the search (Bonferroni, FDR/Benjamini-Hochberg) or validating on data with zero influence on selection (P1-LB2's untouched holdout)
- **Deliverable:** Notes (P1_LB3_Multiple_Testing_Problem.md); worked FWER table, Bonferroni worked example, Benjamini-Hochberg FDR 10-variant worked example, Deflated Sharpe Ratio and Harvey-Liu-Zhu (2016) t-stat>3.0 framework, direct tie to P1-LB2's winner's curse as the n=3 instance of the general formula
- **Carried forward to P1-Build-8 (methodology validator):** flag undisclosed variant counts, missing multiple-testing correction, and "novel" factor claims not framed against Harvey-Liu-Zhu
- **Estimated time:** 1-2 hours
...
### [x] P1-Arch-2: Data flow design — **completed 2026-07-21**
...
- **Key decisions locked:** (1) two parallel flows distinguished — the data plane (real Python DataFrames/objects in RAM) vs. the context plane (only compact structured summaries entering the LLM's prompt), pinning the P1-LA7 "context window ≠ application memory" principle down concretely at every boundary; (2) MCP invocation locus — the LLM orchestrator makes exactly one native tool call (`run_research_pipeline`), and native Python code (not the model) is the actual MCP client looping over every ticker; (3) `FactorSpec` carried forward as a live Python object between stages, re-serialized only for fresh-subagent handoffs; (4) a provisional `FactorMetricsResult` field sketch, flagged for finalization at P1-Arch-4.
- **Deliverable:** Notes (`P1_Arch2_Data_Flow_Design.md`) + inline six-stage sequence diagram rendered in chat
- **Estimated time:** 1-2 hours

### [x] P1-Arch-3: Orchestrator + subagent design — **completed 2026-07-21**
...
- **Key decisions locked:** (1) plan object fully instantiated for P1's actual 7 stages, with two new fields — `plan_assumptions` (concrete per-step downstream beliefs a conflict-check tests against) and a new `"escalated"` status value distinct from `"failed"`; (2) exact validator and memo subagent handoff payload schemas locked field-by-field, replacing P1-Arch-2's provisional "compact handoff" language; (3) full subagent failure-propagation table designed — most importantly, `ValidationResult.severity="blocking"` escalates to a human checkpoint (per P1-LA11's Consequence×Reversibility×Confidence-gap ≥12 threshold) rather than an automatic plan revision, resolving an ambiguity left open since P1-LA7; (4) plan-revision check cadence resolved — hybrid deterministic-trigger design (cheap code-only heuristics after every stage, LLM conflict-check call only when a heuristic trips), closing the open question carried since P1-LA6/P1-Build-7; (5) per-agent data-access/network-egress scope table designed, closing the P1-Arch-1 carried-forward diagram-annotation item per P1-LA10's confused-deputy principle.
