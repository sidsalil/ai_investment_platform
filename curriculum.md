...
>
> **Superseded a 4-project plan.** Old version preserved as
> `curriculum_OLD_2026-05-25.md`. Projects 3-4 kept below as reference only —
> not in active scope.
>
...
> of which existed as explicit lessons in the original plan (they were
> assumed to be picked up implicitly inside build sprints).
>
...
- **Log hours in CONTEXT.md's Hours-Logged Tracker.** The estimate below is untested against your actual pace — recalibrate after the PTO week.
...
**Status:** In progress — Phase 1 (Concept Lessons), Finance track 2/10 complete
...
- **Deliverable:** Notes; a small Jupyter notebook computing both return types on AAPL data
- **Estimated time:** 1-2 hours
...
## Phase 1: Concept Lessons — AI/Agentic Track (~20-25 hours, new)
...
## Phase 1: Concept Lessons — Backtesting Rigor Track (~4-6 hours, new)
...
## Phase 1: Concept Lessons — Evals & Model Evals Track (~1-2 hours, new)
...
### [ ] P1-Arch-3: Orchestrator + subagent design (new)
...
- **Carried-forward requirement (from P1-L2):** Parameterize the universe so NASDAQ-100 ↔ S&P 500 switch is a config change. Source constituents from a stable public source (Wikipedia standard); freeze snapshot date for reproducibility.
- **Located in:** `shared/data/`
- **Tests:** Pytest tests that verify caching works and bad inputs fail cleanly
- **Estimated time:** 5-7 hours (was 3-5; +2 for MCP)
...
- **Estimated time:** 5-7 hours (was 4-6; +1-2 for OOS comparison) - **the hardest build sprint**
...
- **Estimated time:** 4-6 hours (was 3-5; +1 for planning-loop framing)
...
- **Estimated time:** 5-6 hours (was 3-4; +2 for subagent architecture)
...
- **Estimated time:** 4-5 hours (was 2-3; +2 for subagent architecture)

### [ ] P1-Build-10: Tracing / observability layer (new)
- **What you build:** Logging for every tool call and agent decision, so you can debug and explain *why* the agent did what it did
- **Estimated time:** 2-3 hours

### [ ] P1-Build-11: Model comparison eval (new)
...
### [ ] P1-Eval: Build the regression eval set (expanded)
...
- **Carried-forward requirement (from P1-L2):** Include explicit "Universe choice and survivorship-bias acknowledgment" section, expected magnitude, production fix path.
- **Estimated time:** 1 hour

### [ ] P1-Polish-5: Case-study one-pager (new)
...
### [ ] P1-Polish-6: Systems-design writeup (new)
...
# PORTFOLIO COMPLETION & APPLICATION READINESS (new)
...
|---------|---------|--------------|-------|--------|---------|
| P1: Factor Research (Finance) | ☐ 2/10 | ☐ 0/4 | ☐ 0/13 | ☐ 0/6 | ☐ |
...
- Time estimates are a first-pass estimate, not a commitment — you have zero hours logged against the new AI/agentic, backtesting-rigor, or evals tracks yet. Recalibrate using CONTEXT.md's Hours-Logged Tracker starting with the PTO week (2026-07-13).
